...
namespace NuGet.Commands
{
    ...
    public static class RestoreRunner
    {
            ...
            IEnumerable<INuGetLock> projectLocks)
        {
            ...
            var pathToLocks = GetIdToLocks(projectLocks);
        ...
        private static async Task<RestoreSummary> ExecuteAndCommit(
            ...
            Dictionary<string, List<INuGetLock>> locks)
        {
            var result = await Execute(summaryRequest);

            var locksForProject = GetLocksForProject(locks, summaryRequest.Request.Project);

            return await CommitInLock(result, locksForProject);
        }

        public static async Task<RestoreSummary> CommitInLock(RestoreResultPair result, IEnumerable<INuGetLock> locks)
        {
            var activeLocks = new List<INuGetLock>();

            try
            {
                // Get locks
                foreach (var current in locks)
                {
                    await current.AcquireAsync(CancellationToken.None);
                    activeLocks.Add(current);
                }

                // Commit after locking
                return await Commit(result);
            }
            finally
            {
                // Release locks
                foreach (var current in activeLocks)
                {

                            ...
                            totalSteps: 0)))
                    {
                        ...
                        foreach (var project in buildEnabledProjects)
                        {
                            ...
                            if (packageSpec.RestoreMetadata == null)
                            {
                                ...
                                if (referenceContext.Cache.TryGetValue(msBuildMetadata.ProjectPath, out references))
                                {
                                    foreach(var reference in references)
                                    {

                                        if(project.MSBuildProjectPath.Equals(reference.MSBuildProjectPath, 
                                        ...
                                        if (reference.PackageSpecPath == null)
                                        {
                                            ...
                                            pcBasedBuildData.ProjectUniqueName = reference.ProjectName;
                                            ...
                                            pcBasedBuildData.ProjectReferences.AddRange(
                                                reference.ExternalProjectReferences.Select(projRef =>
                                                    new ProjectRestoreReference
                                                    {
                                                        ProjectUniqueName = Path.GetFileNameWithoutExtension(projRef),
                                                        ProjectPath = projRef
                                                    }
                                                )
                                            );

                                            foreach (var projReference in reference.ExternalProjectReferences)
                                            {
                                                pcBasedBuildData.ProjectReferences.Add(
                                                    new ProjectRestoreReference
                                                    {
                                                        ProjectUniqueName = Path.GetFileNameWithoutExtension(projReference),
                                                        ProjectPath = projReference
                                                    });
                                            }

                                            packageConfig.RestoreMetadata = pcBasedBuildData;
